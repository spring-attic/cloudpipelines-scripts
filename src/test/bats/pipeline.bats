...
@test "deployServices should deploy services from pipeline descriptor" {
	...
	assert_success
	assert_line --index 1 'rabbitmq-github-webhook broker'
	assert_line --index 2 'mysql-github-webhook broker'
	assert_line --index 3 'eureka-github-webhook app'
	assert_line --index 4 'stubrunner stubrunner'
}

@test "parsePipelineDescriptor should export an env var with parsed YAML" {
	export PIPELINE_DESCRIPTOR="sc-pipelines-no-services.yml"
	cd "${FIXTURES_DIR}"
	source "${SOURCE_DIR}/pipeline.sh"
