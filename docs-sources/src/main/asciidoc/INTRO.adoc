...
script->customization: Oh, you've provided a URL with tarball with custom scripts
customization->script: Correct, there you go
script->script: Let me unpack it and copy to the sources
script->customization: Anything custom to override in bash?
customization->script: Yeah, let me override the [build_and_upload.sh] script
...
a tarball with additional scripts and copy it to `${sc-pipelines-root}/src/main/bash/`
