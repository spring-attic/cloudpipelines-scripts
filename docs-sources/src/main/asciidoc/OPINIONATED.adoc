...
In this section you will see a Java application going through an opinionated deployment pipeline.
...
In this step we're generating a version of the pipeline, next we're
 running api compatibility, unit, integration and contract tests. Finally we're:
...
First we're running API compatibility check. If there is no production tag (i.e. `prod/${appName}/${version}`)
...
Next, during this phase we're executing a `Maven` build using Maven Wrapper or a `Gradle` build using Gradle Wrapper
, with unit and integration tests. We're also *tagging* the repository with `dev/${appName}/${version}` format. That way in each
...
- starting required (defined in the pipeline descriptor) services in a PAAS
- downloading the artifact from Nexus / Artifactory and we're uploading it to PaaS. We want the application
...
of our application
- from the checked out code we're running the tests available under the `smoke` profile.
- once the tests pass we're searching for the last production release. Once the application
is deployed to production we're tagging it with `prod/${appName}/${version}` tag. If there is no such tag
...
- from the checked out code we're running the tests available under the `e2e` profile
...
- tagging the Git repo with `prod/${appName}/${version}` tag
